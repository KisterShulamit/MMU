...
class CacheUnitTest {
	...
	public CacheUnitTest() throws IOException {
		this.dao = new DaoFileImpl("src/main/resources/datasource.txt", 10);
		this.cacheUnit = new CacheUnit(new LRUAlgoCacheImpl<Long, DataModel<String>>(10));
	}
	...
	void dao() throws IOException {
		...
		/**
		 * Check that Entity(id=100,content=50) was not added because there is an entity
		 * with such an id
		 */
	...
	void cacheUnit() {
		...
		/**
		 * Check if removed elements are not null because capacity is 10 so memory is
		 * full
		 */

		DataModel<String>[] removedElements = cacheUnit.putDataModels(data);

		for (DataModel<String> removedElement : removedElements) {
